...
public:
    ...
    CalcNode(double x, double y, double z, double smth, double vx, double vy, double vz) 
            : x(x), y(y), z(z), smth(smth), vx(vx), vy(vy), vz(vz), timer(0), x0(x), y0(y)
    {
    	a1 = 1;
    	a2 = 1;
    	b1 = 0.1;
    ...
    void move(double tau) {
    	if (timer > (b1 / c1) * y0) {
    		//vx = a2 * y0 * c2 * cos(c2 * timer);
    		//vy = a1 * c1 * cos(b1 * x0 + c1 * timer) - a2 * y0 * c2 * sin(c2 * timer);
    		//vx = a2 * c2 * cos(b1 * y0 + c2 * timer) - a1 * x0 * c1 * sin(c1 * timer);
    		//vy = a1 * x0 * c1 * cos(c1 * timer);
    		vx = -a1 * c1 * cos(b1 * y - c1 * timer) - a2* abs(x0) * cos(b1 * y - c1 * timer);
    		vy = -a2 * x0 * sin(b1 * y - c1 * timer);
	}
	timer += tau;
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double vx = sin(pointX) * sin(pointZ) + sin(pointY);
            double vy = pow(pointZ + pointX, 2);
            double vz = cos(pointY) - pow(pointX, 3); 
